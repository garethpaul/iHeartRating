...
The baseline parses plist/storyboard/SVG files, validates both podspecs, checks `build.sh` shell syntax, verifies rating-view guards for empty, single-item, zero-size, and out-of-range bounce configurations, and reports when Xcode is unavailable.
...
- UI configuration changes should not crash on empty image arrays, single-rating views, zero-sized images, or out-of-range ratings.
