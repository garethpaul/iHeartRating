...
- `make check` runs a static baseline for malformed configuration hardening, CocoaPods metadata, plist/storyboard parsing, build-script syntax, and rating-view edge cases when Xcode is unavailable.
- Rating controls should not crash on empty image arrays, single-rating views, zero-sized images, invalid `maxRating`, out-of-range ratings, or unexpected image assets.
